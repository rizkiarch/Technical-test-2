...
interface StokListProps {
  onRowDoubleClicked: (row: StokModel) => void
}

export type StokListRef = {
  refresh: () => void
};

const StokList = forwardRef<StokListRef, StokListProps>(({ onRowDoubleClicked }, ref) => {
  const [error, setError] = useState<string>();
  const [filters, setFilters] = useState<any>();
  const [rowData, setRowData] = useState<StokModel[]>();
  ...
  const [colDefs] = useState<ColDef<StokModel>[]>([
    { headerName: "Gudang", field: "stk_gud_id", width: 120 },
    { headerName: "Produk", field: "stk_prd_id", width: 120 },
    { headerName: "Qty", field: "stk_qty", width: 120 }
  ]);

  const getStok = async () => {
    try {
      const response = await apiClient(true).get("/v1/stok", {
        params: {
          page: currentPage,
          ...(filters && filters)
        }
      ...
      if (!responseData.status) {
        ...
        setRowData(responseData.data.stoks.data);
        setTotal(responseData.data.stoks.total);
        setPerPage(responseData.data.stoks.per_page);
      }
    } catch (error: any) {
      console.error(error);
