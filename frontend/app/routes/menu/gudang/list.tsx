...
interface GudangListProps {
  onRowDoubleClicked: (row: GudangModel) => void
}

export type GudangListRef = {
  refresh: () => void
};

const GudangList = forwardRef<GudangListRef, GudangListProps>(({ onRowDoubleClicked }, ref) => {
  const [error, setError] = useState<string>();
  const [filters, setFilters] = useState<any>();
  const [rowData, setRowData] = useState<GudangModel[]>();
  ...
  const [colDefs] = useState<ColDef<GudangModel>[]>([
    { headerName: "ID", field: "gud_id", width: 100 },
    { headerName: "Nama Gudang", field: "gud_nama", width: 300 },
    { headerName: "Aktif", field: "gud_aktif", width: 80 }
  ]);

  const getGudang = async () => {
    try {
      const response = await apiClient(true).get("/v1/gudang", {
        params: {
          page: currentPage,
          ...(filters && filters)
        }
      ...
      if (!responseData.status) {
        ...
        setRowData(responseData.data.gudangs.data);
        setTotal(responseData.data.gudangs.total);
        setPerPage(responseData.data.gudangs.per_page);
      }
    } catch (error: any) {
      console.error(error);
