...
const CustomerList = forwardRef<CustomerListRef, CustomerListProps>(({ onRowDoubleClicked }, ref) => {
  ...
  const [colDefs] = useState<ColDef<CustomerModel>[]>([
    ...
    { headerName: "Aktif", field: "cus_aktif", width: 80 },
  ]);

  const getCustomer = async () => {
    try {
      const response = await apiClient(true).get("/v1/customer", {
        params: {
          page: currentPage,
          ...(filters && filters)
        }
      ...
      if (!responseData.status) {
        ...
        setRowData(responseData.data.customers.data);
        setTotal(responseData.data.customers.total);
        setPerPage(responseData.data.customers.per_page);
      }
    } catch (error: any) {
      console.error(error);
